...
//    private static int dynamicSlowdown_lastTimeUnitDestroyed = 0;
    ...
    public static void init() {
        if (Env.isParamTweaker) {
            Ratzass.game.setLocalSpeed(0);
            Ratzass.game.setFrameSkip(500);
            return;
        }

        GameSpeed.disallowToDynamicallySlowdownGameOnFirstFighting();
        Ratzass.game.setLocalSpeed(FASTEST_SPEED);
        Ratzass.game.setFrameSkip(NORMAL_FRAME_SKIP);
    }

    public static void checkIfNeedToSlowDown() {
        if (!oneTimeSlowdownUsed && Ratzass._framesNow <= 1) {
            Ratzass.game.setGUI(frameSkip <= 30);
        }

        if (
            !oneTimeSlowdownUsed
                && Ratzass._framesNow > 60
                && gameSpeed == 0
                && frameSkip >= 30
...
//            CameraCommander.centerCameraNowOnSquadCenter();
    ...
    public static void activateDynamicSlowdown() {
        ...
        Ratzass.game.setLocalSpeed(DYNAMIC_SLOWDOWN_GAME_SPEED);
        Ratzass.game.setFrameSkip(DYNAMIC_SLOWDOWN_FRAME_SKIP);
    ...
    public static void changeSpeedTo(int speed) {
        // if (Ratzass._framesNow > 5) MapSpecificCommander.initialSpeed = false;
        dynamicSlowdownIsActive = false;
        ...
        Ratzass.game.setLocalSpeed(gameSpeed);

        // Turn off GUI for huge game speeds to make it even quicker
        Ratzass.game.setGUI(frameSkip <= 150);
    }
