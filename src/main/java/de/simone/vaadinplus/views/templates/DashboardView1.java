...
import de.simone.vaadinplus.components.list.List2;
import de.simone.vaadinplus.components.list.MessageListItem;
...
public class DashboardView1 extends Main {
    ...
    private RouterLink createSuffix(String label) {
        ...
        link.getElement().setAttribute(ARIA_LABEL, label);
        link.getElement().setAttribute("title", label);
        return link;
    }

    private Component createMarketSummary() {
        Section section = new Section(createChartHeader(), createChartSidebar());
    ...
    private List2 createChartSidebarContent() {
    ...
    private Component renderName(DashboardItem item) {
        ...
        Layout layout2 = new Layout(img, span);
        layout2.addClassNames(Padding.Vertical.SMALL);
        layout2.setAlignItems(Layout.AlignItems.CENTER);
